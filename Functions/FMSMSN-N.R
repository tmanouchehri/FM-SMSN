...
estn<-function(x,x0,g,error,iter.max){
  ...
  st=smsn.mix(ER, nu = 1, g = g, get.init = TRUE, criteria = TRUE,group = TRUE, family = "Normal", calc.im=FALSE)
  nu=1
  p=st$pii[1]
...
goodn<-function(ER,g, pii, mu, sigma2, shape, nu){
  dSN <- function(y, mu = 0, sigma2 = 1, shape=1){
    dens <- 2*dnorm(y, mu, sqrt(sigma2))*pnorm(shape*((y - mu)/sqrt(sigma2)))
    return(dens)
  }
...
garchpern= function(n,xx,piip,arg,wp,mu0p,alpha1p,beta1p){
  ht=yt=rep(0,0)
  zt <-rmix((n), p=piip, family="Normal", arg=arg)
  ht[1] = xx
