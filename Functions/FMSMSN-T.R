...
estt<-function(x,x0,g,error,iter.max){
  dt.ls <- function(x, loc = 0, sigma2 = 1,shape=0, nu = 4){
    d <- (x - loc)/sqrt(sigma2)
  ...
  st=smsn.mix(ER, nu = 1, g = g, get.init = TRUE, criteria = TRUE,group = TRUE, family = "t", calc.im=FALSE)
  nu=st$nu
  p=st$pii[1]
  ...
  while ((criterio > error) && (count <= iter.max)) {
    ...
    for (j in 1:g) {
      ...
      mutij <- Mtij2 * Delta[j] * (Gama[j]^(-1)) * 
        (ER - mu[j])
      A <- mutij/Mtij
      d.vec <- dt.ls(ER, mu[j], sigma2[j], shape[j], 
                     nu)
      if (length(which(d.vec < 1e-10)) > 0) 
        d.vec[which(d.vec < 1e-10)] <- 1e-10
      E = exp(log(2) + nu/2 * log(nu) + lgamma((2 + 
                                                  nu)/2) - (2 + nu)/2 * log(dj + nu + A^2) - 
                (lgamma(nu/2) + log(pi) + 0.5 * log(sigma2[j]) + 
                   log(d.vec)))
      u = exp(log(4) + (nu/2) * log(nu) + lgamma((3 + 
                                                    nu)/2) - (nu + 3)/2 * log(dj + nu) - (lgamma(nu/2) + 
                                                                                            0.5 * log(pi) + 0.5 * log(sigma2[j]) + log(d.vec)) + 
                pt(sqrt((3 + nu)/(dj + nu)) * A, 3 + nu, log.p = TRUE))
...
goodt<-function(ER,g, pii, mu, sigma2, shape, nu){
...
garchpert= function(n,xx,piip,arg,wp,mu0p,alpha1p,beta1p){
  ht=yt=rep(0,0)
  zt <-rmix((n), p=piip, family="t", arg=arg)
  ht[1] =xx
