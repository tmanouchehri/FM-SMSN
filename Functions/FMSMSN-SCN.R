...
#==========================function==========================
garchfscn= function(n,ns){
  ht=yt=rep(0,0)
  zt <-rmix((n+ns), p=piip, family="Skew.cn", arg=list(arg1p,arg2p))
...
estscn<-function(x,x0,g,error,iter.max){
  dSNC <- function(y, mu, sigma2, shape, nu){
    dens <- 2*(nu[1]*dnorm(y, mu, sqrt(sigma2/nu[2]))*pnorm(sqrt(nu[2])*shape*sigma2^(-1/2)*(y-mu)) + (1 - nu[1])*dnorm(y, mu, sqrt(sigma2))*pnorm(shape*sigma2^(-1/2)*(y-mu)))
    return(dens)
  }
  ...
  st=smsn.mix(ER, nu = c(0.5,0.5), g = g, get.init = TRUE, criteria = TRUE,group = TRUE, family = "Skew.cn", calc.im=FALSE)
  nu=st$nu
  p=st$pii[1]
  ...
  LLh<-function(parm){
    ...
    SS=log(d.mixedSNC((x[1]-parm[4])/sqrt(htt[1]),pii,mu,sigma2,shape,nu))
    for(i in 2:n){
      htt[i]<-exp(parm[1])+(1/(1+exp(-parm[2])))*(x[i-1]-parm[4])^2+((1-(1/(1+exp(-parm[2]))))/(1+exp(-parm[3])))*htt[i-1]
      SS=SS+log(d.mixedSNC((x[i]-parm[4])/sqrt(htt[i]),pii,mu,sigma2,shape,nu))
    }
    -SS
  }
  LLhest=0
  ...
  lk <- sum(log(d.mixedSNC(ER, pii, mu, sigma2, shape, nu)))
  n <- length(ER)
  ...
  while ((criterio > error) & (count <= iter.max)) {
    ...
    for (j in 1:g) {
      ...
      mutij <- Mtij2 * Delta[j] * (Gama[j]^(-1)) * 
        (ER - mu[j])
      A <- mutij/Mtij
      d.vec <- dSNC(ER, mu[j], sigma2[j], shape[j], 
                    nu)
      if (length(which(d.vec < 1e-10)) > 0) 
        d.vec[which(d.vec < 1e-10)] <- 1e-10
      u = (2/d.vec) * (nu[1] * nu[2] * dnorm(ER, mu[j], 
                                             sqrt(sigma2[j]/nu[2])) * pnorm(sqrt(nu[2]) * 
                                                                              A, 0, 1) + (1 - nu[1]) * dnorm(ER, mu[j], sqrt(sigma2[j])) * 
                         pnorm(A, 0, 1))
      E = (2/d.vec) * (nu[1] * sqrt(nu[2]) * dnorm(ER, 
                                                   mu[j], sqrt(sigma2[j]/nu[2])) * dnorm(sqrt(nu[2]) * 
                                                                                           A, 0, 1) + (1 - nu[1]) * dnorm(ER, mu[j], sqrt(sigma2[j])) * 
                         dnorm(A, 0, 1))
      d1 <- dSNC(ER, mu[j], sigma2[j], shape[j], nu)
      if (length(which(d1 == 0)) > 0) 
        d1[which(d1 == 0)] <- .Machine$double.xmin
      d2 <- d.mixedSNC(ER, pii, mu, sigma2, shape, nu)
    ...
    logvero.SNC <- function(nu) sum(log(d.mixedSNC(ER, 
                                                   pii, mu, sigma2, shape, nu)))
    nu <- optim(nu.old, logvero.SNC, control = list(fnscale = -1), 
                method = "L-BFGS-B", lower = rep(0.01, 
                                                 2), upper = rep(0.99, 2))$par
    ...
    param <- teta
    teta <- c(mu, Delta, Gama, pii, nu)
    lk1 <- sum(log(d.mixedSNC(ER, pii, mu, sigma2, shape, 
                              nu)))
  ...
  icl <- 0
  for (j in 1:g) icl <- icl + sum(log(pii[j] * dSNC(ER[cl == 
...
goodscn<-function(ER,g, pii, mu, sigma2, shape, nu){
  dSNC <- function(y, mu, sigma2, shape, nu){
    dens <- 2*(nu[1]*dnorm(y, mu, sqrt(sigma2/nu[2]))*pnorm(sqrt(nu[2])*shape*sigma2^(-1/2)*(y-mu)) + (1 - nu[1])*dnorm(y, mu, sqrt(sigma2))*pnorm(shape*sigma2^(-1/2)*(y-mu)))
    return(dens)
  }
...
garchperscn= function(n,xx,piip,arg,wp,mu0p,alpha1p,beta1p){
  ht=yt=rep(0,0)
  zt <-rmix((n), p=piip, family="Skew.cn", arg=arg)
  ht[1] = xx
  yt[1] = wp+zt[1]*sqrt(ht[1]) 
  for(i in 2:(n)){
    ht[i]= mu0p+alpha1p*(yt[i-1]-wp)^2+beta1p*ht[i-1]
