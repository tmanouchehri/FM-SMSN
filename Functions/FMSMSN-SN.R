...
#==========================function==========================
garchfsn= function(n,ns){
  ht=yt=rep(0,0)
  zt <-rmix((n+ns), p=piip, family="Skew.normal", arg=list(arg1p,arg2p))
...
estsn<-function(x,x0,g,error,iter.max){
  dSN <- function(y, mu = 0, sigma2 = 1, shape=1){
    dens <- 2*dnorm(y, mu, sqrt(sigma2))*pnorm(shape*((y - mu)/sqrt(sigma2)))
    return(dens)
  }
  ...
  st=smsn.mix(ER, nu = 1, g = g, get.init = TRUE, criteria = TRUE,group = TRUE, family = "Skew.normal", calc.im=FALSE)
  nu=1
  p=st$pii[1]
  pii=st$pii;mu=st$mu;sigma2=st$sigma2;shape=st$shape
  LLh<-function(parm){
    htt=rep(0,0)
    htt[1]<-exp(parm[1])+(1/(1+exp(-parm[2])))*(x0-parm[4])^2+((1-(1/(1+exp(-parm[2]))))/(1+exp(-parm[3])))*HH[1]
    SS=log(d.mixedSN((x[1]-parm[4])/sqrt(htt[1]),pii,mu,sigma2,shape))
    for(i in 2:n){
      htt[i]<-exp(parm[1])+(1/(1+exp(-parm[2])))*(x[i-1]-parm[4])^2+((1-(1/(1+exp(-parm[2]))))/(1+exp(-parm[3])))*htt[i-1]
      SS=SS+log(d.mixedSN((x[i]-parm[4])/sqrt(htt[i]),pii,mu,sigma2,shape))
    }
    -SS
  }
  LLhest=0
  ...
  lk <- sum(log(d.mixedSN(ER, pii, mu, sigma2, shape)))
  n <- length(ER)
  ...
  while ((criterio > error) & (count <= iter.max)) {
    ...
    for (j in 1:g) {
      ...
      u = rep(1, n)
      d1 <- dSN(ER, mu[j], sigma2[j], shape[j])
      if (length(which(d1 == 0)) > 0) 
        d1[which(d1 == 0)] <- .Machine$double.xmin
      d2 <- d.mixedSN(ER, pii, mu, sigma2, shape)
    ...
    param <- teta
    teta <- c(mu, Delta, Gama, pii)
    lk1 <- sum(log(d.mixedSN(ER, pii, mu, sigma2, shape)))
  ...
  icl <- 0
  for (j in 1:g) icl <- icl + sum(log(pii[j] * dSN(ER[cl == 
                                                        j], mu[j], sigma2[j], shape[j])))
  d <- g * 3 + (g - 1)+4
  aic <- -2 * lk + 2 * d
  bic <- -2 * lk + log(n) * d
...
goodsn<-function(ER,g, pii, mu, sigma2, shape, nu){
  dSN <- function(y, mu = 0, sigma2 = 1, shape=1){
    dens <- 2*dnorm(y, mu, sqrt(sigma2))*pnorm(shape*((y - mu)/sqrt(sigma2)))
    return(dens)
  }
...
garchpersn= function(n,xx,piip,arg,wp,mu0p,alpha1p,beta1p){
  ht=yt=rep(0,0)
  zt <-rmix((n), p=piip, family="Skew.normal", arg=arg)
  ht[1] = xx
  yt[1] = wp+zt[1]*sqrt(ht[1]) 
  for(i in 2:(n)){
    ht[i]= mu0p+alpha1p*(yt[i-1]-wp)^2+beta1p*ht[i-1]
